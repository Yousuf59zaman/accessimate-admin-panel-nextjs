...
import AddEdit from "@/app/components/admin-panel/event/AddEdit";

interface EventItem { id: number; title: string; start_date: string; end_date: string; status: number; }

function EventPageInner() {
  const optionsList = optionsListAcIn();
  const { data, isLoading, permissions, paginationMeta, search, setSearch, status, setStatus, loadData, handleDelete, handleRestore, handleAfterSave, resetPagination } = useCrudPage<EventItem>({ apiEndpoint: "admin/events/all", apiBase: "admin/events", pageSize: 10 });
  ...
  const editHandler = (item: EventItem) => { setEditItem(item); setModalTitle("Edit"); setIsOpenModal(true); };
  const cancelModal = () => { setEditItem(null); setIsOpenModal(false); };
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const receivedData = (item: any) => { handleAfterSave(item as EventItem, modalTitle === "Edit"); setIsOpenModal(false); };
    ...
    <div className="h-full mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 my-6">
      <div className="h-full w-full overflow-auto">
        <div className="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-4 mb-4">
          ...
          {isLoading ? (<div className="w-28 h-10 bg-gray-200 dark:bg-gray-700 rounded-lg animate-pulse" />) : (permissions?.add && (<button onClick={addNew} className="px-4 py-2 bg-sky-500 text-white text-xs font-medium rounded-md hover:bg-sky-600 transition-colors whitespace-nowrap">Create Event</button>))}
        </div>
        <div className="pb-2 flex flex-col justify-between w-full">
          <div className="mt-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800">
            <div className="border-b border-gray-200 dark:border-gray-700"><h4 className="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Events</h4></div>
            <div className="p-4">
                ...
                <table className="table-auto w-full">
                  <thead className="sticky z-10 top-0 bg-gray-50 dark:bg-gray-900">
                    <tr>
                      ...
                      <th className="text-left p-3" style={{ width: "15%" }}><span className="text-gray-800 dark:text-gray-200">Start Date</span></th>
                      <th className="text-left p-3" style={{ width: "15%" }}><span className="text-gray-800 dark:text-gray-200">End Date</span></th>
                    ...
                    <tbody>
                      {data.map((item) => (
                        <tr key={item.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                          ...
                          <td className="text-gray-800 dark:text-gray-200 text-start p-3">{item.start_date}</td>
                          <td className="text-gray-800 dark:text-gray-200 text-start p-3">{item.end_date}</td>
...
export default function EventPage() {
  return (<Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-4 border-sky-500 border-t-transparent rounded-full animate-spin" /></div>}><EventPageInner /></Suspense>);
}
