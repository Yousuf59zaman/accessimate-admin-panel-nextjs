...
function FeaturesPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<FeatureItem>({
    apiEndpoint: "admin/features/all",
    apiBase: "admin/features",
    pageSize: 10,
  });
  ...
  const editHandler = (item: FeatureItem) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as FeatureItem, modalTitle === "Edit");
    setIsOpenModal(false);
  };
