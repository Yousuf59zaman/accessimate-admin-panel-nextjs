...
function PaymentGatewaysPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<PaymentGateway>({
    apiEndpoint: "admin/payment-gateways/all",
    apiBase: "admin/payment-gateways",
    pageSize: 10,
  });
  ...
  const editHandler = (item: PaymentGateway) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as PaymentGateway, modalTitle === "Edit");
    setIsOpenModal(false);
  };
