...
function PartnersPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<Partner>({
    apiEndpoint: "admin/partners/all",
  ...
  const editHandler = (item: Partner) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as Partner, modalTitle === "Edit");
    setIsOpenModal(false);
  };
                      ...
                      {data.map((item) => (
                        ...
                        >
                          <td className="text-gray-800 dark:text-gray-200 text-start p-3">
                            <div className="flex items-center gap-2">
                              <img
                                src={item.logo || "/svg/not-found-img.svg"}
