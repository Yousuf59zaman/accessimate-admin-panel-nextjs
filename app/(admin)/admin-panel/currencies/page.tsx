...
import AddEdit from "@/app/components/admin-panel/currencies/AddEdit";
...
function CurrenciesPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useCrudPage<Currency>({
    apiEndpoint: "admin/currencies/all",
    apiBase: "admin/currencies",
    pageSize: 10,
  });
  ...
  const editHandler = (item: Currency) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as Currency, modalTitle === "Edit");
    setIsOpenModal(false);
  };
