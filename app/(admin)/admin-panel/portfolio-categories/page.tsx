...
import AddEdit from "@/app/components/admin-panel/portfolio-categories/AddEdit";

interface PortfolioCategory { id: number; title: string; slug: string; status: number; }

function PortfolioCategoriesPageInner() {
  const optionsList = optionsListAcIn();
  const { data, isLoading, permissions, paginationMeta, search, setSearch, status, setStatus, loadData, handleDelete, handleRestore, handleAfterSave, resetPagination } = useCrudPage<PortfolioCategory>({ apiEndpoint: "admin/portfolio-categories/all", apiBase: "admin/portfolio-categories", pageSize: 10 });
  ...
  const [editItem, setEditItem] = useState<PortfolioCategory | null>(null);
  const addNew = () => { setEditItem(null); setModalTitle("Create"); setIsOpenModal(true); };
  const editHandler = (item: PortfolioCategory) => { setEditItem(item); setModalTitle("Edit"); setIsOpenModal(true); };
  const cancelModal = () => { setEditItem(null); setIsOpenModal(false); };
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const receivedData = (item: any) => { handleAfterSave(item as PortfolioCategory, modalTitle === "Edit"); setIsOpenModal(false); };
      ...
      <div className="h-full w-full overflow-auto">
        ...
        <div className="pb-2 flex flex-col justify-between w-full">
          <div className="mt-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800">
            <div className="border-b border-gray-200 dark:border-gray-700"><h4 className="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Portfolio Categories</h4></div>
            <div className="p-4">
...
export default function PortfolioCategoriesPage() {
  return (<Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="w-10 h-10 border-4 border-sky-500 border-t-transparent rounded-full animate-spin" /></div>}><PortfolioCategoriesPageInner /></Suspense>);
}
