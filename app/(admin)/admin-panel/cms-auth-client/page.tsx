...
import { optionsAcTr } from "@/app/helpers/globalFunctions";
...
function CmsAuthClientPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<CmsAuthClient>({
    apiEndpoint: "admin/auth-client/all",
    apiBase: "admin/auth-client",
    pageSize: 10,
    initialStatus: optionsList[0],
  });
  ...
  const editHandler = (item: CmsAuthClient) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as CmsAuthClient, modalTitle === "Edit");
    setIsOpenModal(false);
  };
