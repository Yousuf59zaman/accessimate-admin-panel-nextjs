...
function SocialLinksPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<SocialLink>({
    apiEndpoint: "admin/social-links/all",
    apiBase: "admin/social-links",
    pageSize: 10,
  });
  ...
  const editHandler = (item: SocialLink) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as SocialLink, modalTitle === "Edit");
    setIsOpenModal(false);
  };
