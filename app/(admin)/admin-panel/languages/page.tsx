...
import AddEdit from "@/app/components/admin-panel/languages/AddEdit";
...
function LanguagesPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<Language>({
    apiEndpoint: "admin/languages/all",
    apiBase: "admin/languages",
    pageSize: 10,
  });
  ...
  const editHandler = (item: Language) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as Language, modalTitle === "Edit");
    setIsOpenModal(false);
  };
