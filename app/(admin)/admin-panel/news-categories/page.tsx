...
function NewsCategoriesPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<NewsCategory>({
    apiEndpoint: "admin/news-categories/all",
    apiBase: "admin/news-categories",
    pageSize: 10,
  });
  ...
  const editHandler = (item: NewsCategory) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as NewsCategory, modalTitle === "Edit");
    setIsOpenModal(false);
  };
