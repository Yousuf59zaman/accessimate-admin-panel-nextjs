...
function FootersPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useCrudPage<FooterItem>({
    apiEndpoint: "admin/footers/all",
    apiBase: "admin/footers",
    pageSize: 10,
  });
  ...
  const editHandler = (item: FooterItem) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as FooterItem, modalTitle === "Edit");
    setIsOpenModal(false);
  };
