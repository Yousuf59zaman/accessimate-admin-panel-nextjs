...
function SmsTemplatesPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<SmsTemplate>({
    apiEndpoint: "admin/sms-templates/all",
    apiBase: "admin/sms-templates",
    pageSize: 10,
  });
  ...
  const editHandler = (item: SmsTemplate) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as SmsTemplate, modalTitle === "Edit");
    setIsOpenModal(false);
  };
