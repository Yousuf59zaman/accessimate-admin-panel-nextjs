...
function TutorialPageInner() {
  ...
  // Auto-open first panel when data loads
  useMemo(() => {
    if (groupedData.length > 0) {
      const firstKey = `${groupedData[0].id}-${groupedData[0].title}`;
      setOpenPanels((prev) => {
                      ...
                      return (
                        ...
                        >
                          <button
                            ...
                            className={`w-full flex items-center justify-between p-4 text-left font-semibold text-gray-800 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${isOpen ? "bg-sky-500 text-white hover:bg-sky-600" : ""}`}
                          >
                            <span>{group.title}</span>
                            <i
                              className={`fa-solid fa-chevron-${isOpen ? "up" : "down"} ${isOpen ? "text-white" : "text-gray-500 dark:text-gray-400"}`}
                            />
                          </button>
                          {isOpen && (
                            <div className="border-t border-gray-300 dark:border-gray-600 p-4">
