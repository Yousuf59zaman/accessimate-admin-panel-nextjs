...
function TutorialCategoriesPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<TutorialCategory>({
    apiEndpoint: "admin/tutorial-categories/all",
    apiBase: "admin/tutorial-categories",
    pageSize: 10,
  });
  ...
  const editHandler = (item: TutorialCategory) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as TutorialCategory, modalTitle === "Edit");
    setIsOpenModal(false);
  };
