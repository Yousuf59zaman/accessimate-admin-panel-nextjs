...
function MetasPageInner() {
  ...
  const {
    ...
    handleAfterSave,
    resetPagination,
  } = useAdminCrud<MetaItem>({
    apiEndpoint: "admin/metas/all",
    apiBase: "admin/metas",
    pageSize: 10,
  });
  ...
  const editHandler = (item: MetaItem) => {
    setEditItem(item);
    setModalTitle("Edit");
    setIsOpenModal(true);
  };
  ...
  const receivedData = (item: any) => {
    handleAfterSave(item as MetaItem, modalTitle === "Edit");
    setIsOpenModal(false);
  };
        ...
        <tr key={idx}>
          <td className="text-gray-800 dark:text-gray-200 text-start p-3">
            <div className="flex items-center gap-2">
              <div className="w-[90px] h-[80px] bg-gray-200 dark:bg-gray-700 rounded animate-pulse" />
                              ...
                              <div className="m-2 w-[90px] h-[80px] bg-white rounded-[5px] flex items-center justify-center text-[#ccc] border border-gray-300">
                                {item.og_image && !imageErrors[item.id] ? (
                                  <img
                                    src={item.og_image}
                                    alt={item.title}
                                    className="block border-gray-300 rounded-lg h-[80px] p-2"
