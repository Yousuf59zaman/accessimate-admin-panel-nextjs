...
interface CustomerReview { id?: number; name?: string; designation?: string; review?: string; rating?: string; status?: number; }
interface FormData { name: string; designation: string; review: string; rating: string; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: CustomerReview; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: CustomerReview | null; modalTitle: string; onClose: () => void; onSave: (item: CustomerReview) => void; }

const emptyForm: FormData = { name: "", designation: "", review: "", rating: "", status: 0 };
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  useEffect(() => {
    if (item && Object.keys(item).length > 0) { setValidationErrors({}); setFormData({ name: item.name || "", designation: item.designation || "", review: item.review || "", rating: item.rating || "", status: item.status || 0 }); setIsChecked(item.status === 1); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/customer-reviews/${item?.id}` : "admin/customer-reviews", { method: isEdit ? "PUT" : "POST", body: submitData });
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 animate-modal-enter max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} Customer Review</h4></div>
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Name</label>
              <div className="flex-auto">
                <input type="text" value={formData.name} onChange={(e) => updateField("name", e.target.value)} placeholder="i.e. John Doe" className={inputClass("name")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, name: "" }))} />
                {validationErrors.name && <p className="text-red-500 text-sm mt-1">{validationErrors.name}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Designation</label>
              <div className="flex-auto">
                <input type="text" value={formData.designation} onChange={(e) => updateField("designation", e.target.value)} placeholder="i.e. CEO" className={inputClass("designation")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, designation: "" }))} />
                {validationErrors.designation && <p className="text-red-500 text-sm mt-1">{validationErrors.designation}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Review</label>
              <div className="flex-auto">
                <textarea value={formData.review} onChange={(e) => updateField("review", e.target.value)} className={inputClass("review")} rows={3} onFocus={() => setValidationErrors((prev) => ({ ...prev, review: "" }))} />
                {validationErrors.review && <p className="text-red-500 text-sm mt-1">{validationErrors.review}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Rating</label>
              <div className="flex-auto">
                <input type="number" step="0.01" value={formData.rating} onChange={(e) => updateField("rating", e.target.value)} placeholder="i.e. 5" className={inputClass("rating")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, rating: "" }))} />
