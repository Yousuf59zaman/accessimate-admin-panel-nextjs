...
interface FaqItem { id?: number; question?: string; answer?: string; status?: number; }
interface FormData { question: string; answer: string; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: FaqItem; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: FaqItem | null; modalTitle: string; onClose: () => void; onSave: (item: FaqItem) => void; }

const emptyForm: FormData = { question: "", answer: "", status: 0 };
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  useEffect(() => {
    if (item && Object.keys(item).length > 0) { setValidationErrors({}); setFormData({ question: item.question || "", answer: item.answer || "", status: item.status || 0 }); setIsChecked(item.status === 1); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/faqs/${item?.id}` : "admin/faqs", { method: isEdit ? "PUT" : "POST", body: submitData });
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 animate-modal-enter max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} FAQ</h4></div>
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Question</label>
              <div className="flex-auto">
                <input type="text" value={formData.question} onChange={(e) => updateField("question", e.target.value)} placeholder="i.e. What is this?" className={inputClass("question")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, question: "" }))} />
                {validationErrors.question && <p className="text-red-500 text-sm mt-1">{validationErrors.question}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Answer</label>
              <div className="flex-auto">
                <textarea value={formData.answer} onChange={(e) => updateField("answer", e.target.value)} placeholder="i.e. This is..." className={inputClass("answer")} rows={3} onFocus={() => setValidationErrors((prev) => ({ ...prev, answer: "" }))} />
                {validationErrors.answer && <p className="text-red-500 text-sm mt-1">{validationErrors.answer}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
