...
interface PlanItem { id?: number; name?: string; slug?: string; serials?: number; description?: string; status?: number; features?: unknown[]; prices?: unknown[]; }
interface FormData { name: string; slug: string; serials: number; description: string; status: number; }
...
const emptyForm: FormData = { name: "", slug: "", serials: 0, description: "", status: 0 };
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  useEffect(() => {
    if (item && Object.keys(item).length > 0) { 
        setValidationErrors({}); 
        setFormData({ name: item.name || "", slug: item.slug || "", serials: item.serials || 0, description: item.description || "", status: item.status || 0 }); 
  ...
  const handleSubmit = async () => {
    ...
    try {
      const submitData = { ...formData, status: isChecked ? 1 : 0, features: [], prices: [] };
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl mx-4 animate-modal-enter max-h-[90vh] flex flex-col">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700 shrink-0"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} Plans</h4></div>
          <div className="p-6 overflow-y-auto">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="col-span-1 sm:col-span-2">
                <label className="font-semibold block mb-1 text-gray-900 dark:text-white">Slug</label>
                <input type="text" value={formData.slug} onChange={(e) => updateField("slug", e.target.value)} placeholder="i.e. web-development" className={inputClass("slug")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, slug: "" }))} />
                {validationErrors.slug && <p className="text-red-500 text-sm mt-1">{validationErrors.slug}</p>}
              </div>
              <div className="col-span-1">
                <label className="font-semibold block mb-1 text-gray-900 dark:text-white">Name</label>
                <input type="text" value={formData.name} onChange={(e) => updateField("name", e.target.value)} placeholder="i.e. Web Development" className={inputClass("name")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, name: "" }))} />
                {validationErrors.name && <p className="text-red-500 text-sm mt-1">{validationErrors.name}</p>}
              </div>
              <div className="col-span-1 sm:col-span-2">
                <label className="font-semibold block mb-1 text-gray-900 dark:text-white">Description</label>
                <textarea value={formData.description} onChange={(e) => updateField("description", e.target.value)} placeholder="i.e. Write your description here" className={inputClass("description")} rows={2} onFocus={() => setValidationErrors((prev) => ({ ...prev, description: "" }))} />
                {validationErrors.description && <p className="text-red-500 text-sm mt-1">{validationErrors.description}</p>}
              </div>
              <div className="col-span-1">
                <label className="font-semibold block mb-1 text-gray-900 dark:text-white">Hierarchy</label>
                <input type="number" value={formData.serials || ""} onChange={(e) => updateField("serials", parseInt(e.target.value, 10))} className={inputClass("serials")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, serials: "" }))} />
                {validationErrors.serials && <p className="text-red-500 text-sm mt-1">{validationErrors.serials}</p>}
              </div>
              ...
              {/* TBD Features / Prices array editors (requires heavy drawer implementation like Nuxt) */}
              <div className="col-span-1 sm:col-span-3 text-sm text-gray-500 italic mt-4 border-t pt-4">Features and Prices array management to be fully implemented. Currently saving basic plan details.</div>
            </div>
          </div>
