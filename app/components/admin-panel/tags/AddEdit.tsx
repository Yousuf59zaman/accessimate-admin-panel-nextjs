...
export default function AddEdit({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = isEdit ? `admin/tags/${item?.id}` : "admin/tags";
          ...
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <input
                  type="text"
                  value={formData.tag_title}
                  onChange={(e) => updateField("tag_title", e.target.value)}
                  placeholder="i.e. Example Tag"
                  className={inputClass("tag_title")}
                  autoComplete="off"
                  onFocus={() =>
                    setValidationErrors((prev) => ({ ...prev, tag_title: "" }))
                  }
                />
                {validationErrors.tag_title && (
