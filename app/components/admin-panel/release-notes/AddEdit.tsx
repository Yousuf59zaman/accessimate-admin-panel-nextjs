...
interface ReleaseNote { id?: number; title?: string; version?: string; description?: string; status?: number; }
interface FormData { title: string; version: string; description: string; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: ReleaseNote; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: ReleaseNote | null; modalTitle: string; onClose: () => void; onSave: (item: ReleaseNote) => void; }

const emptyForm: FormData = { title: "", version: "", description: "", status: 0 };
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  useEffect(() => {
    if (item && Object.keys(item).length > 0) { setValidationErrors({}); setFormData({ title: item.title || "", version: item.version || "", description: item.description || "", status: item.status || 0 }); setIsChecked(item.status === 1); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/release-notes/${item?.id}` : "admin/release-notes", { method: isEdit ? "PUT" : "POST", body: submitData });
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 animate-modal-enter max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} Release Note</h4></div>
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Title</label>
              <div className="flex-auto">
                <input type="text" value={formData.title} onChange={(e) => updateField("title", e.target.value)} placeholder="i.e. v2.0 Release" className={inputClass("title")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, title: "" }))} />
                {validationErrors.title && <p className="text-red-500 text-sm mt-1">{validationErrors.title}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Version</label>
              <div className="flex-auto">
                <input type="text" value={formData.version} onChange={(e) => updateField("version", e.target.value)} placeholder="i.e. 2.0.0" className={inputClass("version")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, version: "" }))} />
                {validationErrors.version && <p className="text-red-500 text-sm mt-1">{validationErrors.version}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
