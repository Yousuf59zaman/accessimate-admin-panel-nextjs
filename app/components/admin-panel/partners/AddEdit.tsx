...
const emptyForm: FormData = { name: "", short_desc: "", logo: "", status: 0 };
const skipValidations = ["status", "logo"];
...
export default function AddEdit({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If logo is a URL (already uploaded), don't send it
      if (
        typeof submitData.logo === "string" &&
        (submitData.logo as string).includes("http")
      ) {
        delete submitData.logo;
      }
      const isEdit = modalTitle !== "Create";
      const result = await fetchAdmin<SubmitApiResponse>(
        isEdit ? `admin/partners/${item?.id}` : "admin/partners",
                ...
                <div className="w-full mt-2">
                  {formData.logo ? (
                    <img
                      src={formData.logo}
                      alt="Partner Logo"
                      className="w-32 h-32 object-contain rounded-md bg-gray-50 dark:bg-gray-700/50 p-2"
                    />
                  ) : (
                    <div className="w-32 h-32 bg-gray-100 dark:bg-gray-700 rounded-md flex items-center justify-center text-gray-400">
            ...
            <div className="flex items-center gap-4">
              <div className="flex-auto">
                ...
                <textarea
                  ...
                  placeholder="i.e. Write your short description here"
                  className={inputClass("short_desc")}
                  rows={7}
                  onFocus={() =>
                    setValidationErrors((prev) => ({ ...prev, short_desc: "" }))
                  }
                />
