...
interface NewsCategory { id?: number; title?: string; slug?: string; parent_id?: number | string; status?: number; }
interface FormData { title: string; slug: string; parent_id: string | number; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: NewsCategory; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: NewsCategory | null; modalTitle: string; onClose: () => void; onSave: (item: NewsCategory) => void; allData?: NewsCategory[]; }
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave, allData }: AddEditProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/news-categories/${item?.id}` : "admin/news-categories", { method: isEdit ? "PUT" : "POST", body: submitData });
          ...
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <input type="text" value={formData.title} onChange={(e) => updateField("title", e.target.value)} placeholder="i.e. Technology" className={inputClass("title")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, title: "" }))} />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <input type="text" value={formData.slug} onChange={(e) => updateField("slug", e.target.value)} placeholder="i.e. technology" className={inputClass("slug")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, slug: "" }))} />
