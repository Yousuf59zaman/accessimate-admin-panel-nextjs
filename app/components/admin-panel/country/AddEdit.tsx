...
interface FormData { nationality: string; iso_code: string; num_code: string; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: Country; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: Country | null; modalTitle: string; onClose: () => void; onSave: (item: Country) => void; }

const emptyForm: FormData = { nationality: "", iso_code: "", num_code: "", status: 0 };
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  useEffect(() => {
    if (item && Object.keys(item).length > 0) { setValidationErrors({}); setFormData({ nationality: item.nationality || "", iso_code: item.iso_code || "", num_code: item.num_code || "", status: item.status || 0 }); setIsChecked(item.status === 1); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/countries/${item?.id}` : "admin/countries", { method: isEdit ? "PUT" : "POST", body: submitData });
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 animate-modal-enter max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} Country</h4></div>
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Nationality</label>
              <div className="flex-auto">
                <input type="text" value={formData.nationality} onChange={(e) => updateField("nationality", e.target.value)} placeholder="i.e. American" className={inputClass("nationality")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, nationality: "" }))} />
                {validationErrors.nationality && <p className="text-red-500 text-sm mt-1">{validationErrors.nationality}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">ISO Code</label>
              <div className="flex-auto">
                <input type="text" value={formData.iso_code} onChange={(e) => updateField("iso_code", e.target.value)} placeholder="i.e. US" className={inputClass("iso_code")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, iso_code: "" }))} />
                {validationErrors.iso_code && <p className="text-red-500 text-sm mt-1">{validationErrors.iso_code}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Num Code</label>
              <div className="flex-auto">
                <input type="text" value={formData.num_code} onChange={(e) => updateField("num_code", e.target.value)} placeholder="i.e. +1" className={inputClass("num_code")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, num_code: "" }))} />
                {validationErrors.num_code && <p className="text-red-500 text-sm mt-1">{validationErrors.num_code}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
