...
interface AddEditProps {
  isOpen: boolean;
  item: SmsTemplate | null;
  modalTitle: string;
  onClose: () => void;
  onSave: (item: SmsTemplate) => void;
}

const emptyForm: FormData = { name: "", slug: "", sms_body: "", status: 0 };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(
        isEdit ? `admin/sms-templates/${item?.id}` : "admin/sms-templates",
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <textarea
                  ...
                  className={inputClass("sms_body")}
                  rows={3}
                  onFocus={() =>
                    setValidationErrors((prev) => ({ ...prev, sms_body: "" }))
                  }
                  placeholder="i.e. Welcome to our platform"
                />
                {validationErrors.sms_body && (
                  <p className="text-red-500 text-sm mt-1">
                    {validationErrors.sms_body}
                  </p>
                )}
