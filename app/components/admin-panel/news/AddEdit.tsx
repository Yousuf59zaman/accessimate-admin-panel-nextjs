...
interface NewsItem { id?: number; title?: string; slug?: string; short_description?: string; description?: string; status?: number; }
interface FormData { title: string; slug: string; short_description: string; description: string; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: NewsItem; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: NewsItem | null; modalTitle: string; onClose: () => void; onSave: (item: NewsItem) => void; }
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/news/${item?.id}` : "admin/news", { method: isEdit ? "PUT" : "POST", body: submitData });
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 animate-modal-enter max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} News</h4></div>
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Title</label>
              <div className="flex-auto">
                <input type="text" value={formData.title} onChange={(e) => updateField("title", e.target.value)} placeholder="i.e. Breaking News" className={inputClass("title")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, title: "" }))} />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <input type="text" value={formData.slug} onChange={(e) => updateField("slug", e.target.value)} placeholder="i.e. breaking-news" className={inputClass("slug")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, slug: "" }))} />
