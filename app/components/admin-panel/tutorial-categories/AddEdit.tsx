...
interface TutorialCategory { id?: number; title?: string; slug?: string; parent_id?: number | string; status?: number; }
interface FormData { title: string; slug: string; parent_id: string | number; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: TutorialCategory; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: TutorialCategory | null; modalTitle: string; onClose: () => void; onSave: (item: TutorialCategory) => void; allData?: TutorialCategory[]; }
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave, allData }: AddEditProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/tutorial-categories/${item?.id}` : "admin/tutorial-categories", { method: isEdit ? "PUT" : "POST", body: submitData });
          ...
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <input type="text" value={formData.title} onChange={(e) => updateField("title", e.target.value)} placeholder="i.e. Beginner" className={inputClass("title")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, title: "" }))} />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <input type="text" value={formData.slug} onChange={(e) => updateField("slug", e.target.value)} placeholder="i.e. beginner" className={inputClass("slug")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, slug: "" }))} />
