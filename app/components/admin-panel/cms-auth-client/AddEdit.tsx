...
interface FormData { name: string; client_id: string; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: CmsAuthClient; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: CmsAuthClient | null; modalTitle: string; onClose: () => void; onSave: (item: CmsAuthClient) => void; }

const emptyForm: FormData = { name: "", client_id: "", status: 0 };
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  useEffect(() => {
    if (item && Object.keys(item).length > 0) { setValidationErrors({}); setFormData({ name: item.name || "", client_id: item.client_id || "", status: item.status || 0 }); setIsChecked(item.status === 1); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/cms-auth-client/${item?.id}` : "admin/cms-auth-client", { method: isEdit ? "PUT" : "POST", body: submitData });
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 animate-modal-enter max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} CMS Auth Client</h4></div>
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Name</label>
              <div className="flex-auto">
                <input type="text" value={formData.name} onChange={(e) => updateField("name", e.target.value)} placeholder="i.e. Client App" className={inputClass("name")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, name: "" }))} />
                {validationErrors.name && <p className="text-red-500 text-sm mt-1">{validationErrors.name}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Client ID</label>
              <div className="flex-auto">
                <input type="text" value={formData.client_id} onChange={(e) => updateField("client_id", e.target.value)} placeholder="i.e. abc123" className={inputClass("client_id")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, client_id: "" }))} />
                {validationErrors.client_id && <p className="text-red-500 text-sm mt-1">{validationErrors.client_id}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
