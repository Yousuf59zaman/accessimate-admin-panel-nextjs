...
interface EventItem { id?: number; title?: string; slug?: string; start_date?: string; end_date?: string; short_description?: string; description?: string; status?: number; }
interface FormData { title: string; slug: string; start_date: string; end_date: string; short_description: string; description: string; status: number; }
interface SubmitApiResponse { status: boolean; message?: string; data: EventItem; }
interface FetchError extends Error { response?: Response; data?: { status?: boolean; message?: string; data?: Record<string, string[]> }; }
interface AddEditProps { isOpen: boolean; item: EventItem | null; modalTitle: string; onClose: () => void; onSave: (item: EventItem) => void; }

const emptyForm: FormData = { title: "", slug: "", start_date: "", end_date: "", short_description: "", description: "", status: 0 };
...
export default function AddEdit({ isOpen, item, modalTitle, onClose, onSave }: AddEditProps) {
  ...
  useEffect(() => {
    if (item && Object.keys(item).length > 0) { setValidationErrors({}); setFormData({ title: item.title || "", slug: item.slug || "", start_date: item.start_date || "", end_date: item.end_date || "", short_description: item.short_description || "", description: item.description || "", status: item.status || 0 }); setIsChecked(item.status === 1); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await fetchAdmin<SubmitApiResponse>(isEdit ? `admin/events/${item?.id}` : "admin/events", { method: isEdit ? "PUT" : "POST", body: submitData });
    ...
    <>
      <div className="fixed inset-0 z-9999 flex items-center justify-center">
        ...
        <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg mx-4 animate-modal-enter max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-center w-full gap-2 p-4 border-b border-gray-200 dark:border-gray-700"><h4 className="text-xl font-semibold text-gray-900 dark:text-white">{modalTitle} Event</h4></div>
          <div className="grid grid-cols-1 gap-4 p-6">
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Title</label>
              <div className="flex-auto">
                <input type="text" value={formData.title} onChange={(e) => updateField("title", e.target.value)} placeholder="i.e. Annual Meetup" className={inputClass("title")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, title: "" }))} />
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-auto">
                <input type="text" value={formData.slug} onChange={(e) => updateField("slug", e.target.value)} placeholder="i.e. annual-meetup" className={inputClass("slug")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, slug: "" }))} />
                {validationErrors.slug && <p className="text-red-500 text-sm mt-1">{validationErrors.slug}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Start Date</label>
              <div className="flex-auto">
                <input type="date" value={formData.start_date} onChange={(e) => updateField("start_date", e.target.value)} className={inputClass("start_date")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, start_date: "" }))} />
                {validationErrors.start_date && <p className="text-red-500 text-sm mt-1">{validationErrors.start_date}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">End Date</label>
              <div className="flex-auto">
                <input type="date" value={formData.end_date} onChange={(e) => updateField("end_date", e.target.value)} className={inputClass("end_date")} autoComplete="off" onFocus={() => setValidationErrors((prev) => ({ ...prev, end_date: "" }))} />
                {validationErrors.end_date && <p className="text-red-500 text-sm mt-1">{validationErrors.end_date}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
              <label className="font-semibold w-24 text-gray-900 dark:text-white">Short Description</label>
              <div className="flex-auto">
                <textarea value={formData.short_description} onChange={(e) => updateField("short_description", e.target.value)} className={inputClass("short_description")} rows={3} onFocus={() => setValidationErrors((prev) => ({ ...prev, short_description: "" }))} />
                {validationErrors.short_description && <p className="text-red-500 text-sm mt-1">{validationErrors.short_description}</p>}
              </div>
            </div>
            <div className="flex items-center gap-4">
